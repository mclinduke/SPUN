    ...
    configureServer(server) {
      server.middlewares.use('/api/discogs', async (req, res) => {
        try {
          ...
          if (!/^(database\/search|releases\/\d+|masters\/\d+|masters\/\d+\/versions|users\/[^/]+\/collection\/folders\/\d+\/releases)$/.test(path)) { res.statusCode = 403; res.end(JSON.stringify({ error: 'path not allowed' })); return }
